...
public class AbortJobListener extends LopVmListener {
    ...
    public void processAbortJobPacket(AbortJob abortJob) {
        ...
        if (null == vmPassword || null == jobId) {
            ...
            abortJob.setError(new LopXmppError(XMPPError.Condition.bad_request, LinkedProcess.LopErrorType.MALFORMED_PACKET, errorMessage));
        } else if (!((XmppVirtualMachine) this.xmppClient).checkVmPassword(vmPassword)) {
            abortJob.setType(IQ.Type.ERROR);
            abortJob.setError(new LopXmppError(XMPPError.Condition.not_authorized, LinkedProcess.LopErrorType.WRONG_VM_PASSWORD, null));
        } else {
            try {
                ...
                abortJob.setType(IQ.Type.ERROR);
                abortJob.setError(new LopXmppError(XMPPError.Condition.interna_server_error, LinkedProcess.LopErrorType.INTERNAL_ERROR, e.getMessage()));
            } catch (JobNotFoundException e) {
                abortJob.setType(IQ.Type.ERROR);
                abortJob.setError(new LopXmppError(XMPPError.Condition.item_not_found, LinkedProcess.LopErrorType.JOB_NOT_FOUND, e.getMessage()));
            }
        }
