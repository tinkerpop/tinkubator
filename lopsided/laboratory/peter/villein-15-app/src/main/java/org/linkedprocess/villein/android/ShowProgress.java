	...
	private void findPrimes(Bundle context) throws Exception {
		Thread findPrimesThread = new Thread(new Runnable() {
			...
			public void run() {
				...
				status("connecting");
				try {
					Prefs prefs = Villein.prefs;
					PrimeFinderAsynchWithProgress.init(prefs.username,
							prefs.password, prefs.server, prefs.port, null);
					// ////////////// ALLOCATE FARMS
					...
					status("spawning " + 1 + " VMs");
					PrimeFinderAsynchWithProgress.spawnVms(1);
